...
Fourteen citations worked 14 September 2026, then S1 and S2 re-opened after independent direct access to both primary sources. `FACT_CHECK.md` §1 carries the packet.

**The method failed once, and it is worth recording how.** Every primary host is refused at this environment's gateway, so twelve of the fourteen rest on indexed retrieval of the sources' own text. For S1 that returned one of the two mechanisms the DOJ release describes — money temporarily wired into investors' accounts to fake proof of funds — and not the second, which is that Onofrio loaned investors their down-payment money, kept those loans off their financial statements, and left the promissory notes unrecorded. Reading that absence as evidence, an accurate manuscript sentence was replaced with a narrower one and a justification was written into the fact-check file saying nothing supported the original. Both the change and the justification were wrong. The sentence is restored and the claim withdrawn.

The lesson is narrow and worth keeping: indexed retrieval can establish that a source says something. It cannot establish that a source says nothing further. Status labels in `FACT_CHECK.md` §1 now separate **Verified (direct)**, where the document was read, from **Provisional (indexed)**, where it was not — and twelve of the fourteen are provisional.

**S2 stands, with two refinements.** The indication-specific correction was right and necessary: the label pools nothing across indications and 1,441 is the psoriatic arthritis exposed population. The endnote now cites the December 2025 prescribing information and section 5.3 rather than presenting a Drugs@FDA supplement number as current. And a phrase of mine, "in the psoriasis trials it runs the other way," has been cut as imprecise — the psoriasis event rate was lower on the drug (0.1%) than on placebo (0.2%), so the direction did not reverse; what differed was severity. The manuscript now reports both events and characterises neither.
...
- **Access limits** — the figure is verified from the source's own abstract; only the surrounding article is unopened. The figure stands, and where the manuscript asserts nothing beyond the abstract, further reading is optional.
- **Provisional attributions** — a claim resting on secondary reporting that no primary record has confirmed. **No additional evidence was obtained for either in this pass.** A legal disposition needs the primary court record; other factual details need directly inspected, reliable supporting evidence from any sound source.
...
2. **S6, full article — optional, not outstanding.** All six year-percentages are **verified from the original abstract**, and the manuscript asserts nothing beyond them, so the claim as written is closed. Reading the full article is **optional** and becomes necessary only if a manuscript claim comes to depend on material outside the abstract. Listed to record the access limit, not to hold the figure open, and independent of item 1 — S5's subgroup-context check is a separate question about a separate figure.
3. **S14, Jones's final disposition — provisional; requires the primary court record.** Two of three dispositions rest on complete appellate opinions; the third rests on an AP report of a guilty plea, with a federal opinion confirming only that an earlier plea was withdrawn. It is a legal disposition, and news coverage is never sole authority for one under the standard the other two were held to, so nothing short of the court record closes it. This is the one open item where a sentence in the book — "three men were later convicted or pleaded guilty" — is not yet fully sourced.
4. **S14, discovery date and console brand — provisional; requires directly inspected evidence, not necessarily a court record.** 24 July 2013 and the PlayStation 3 rest on news reporting alone and **neither was resolved in this pass.** The appellate opinions describe a video game system without naming a brand and date the burglary to the night of 23 July; that is consistent with a discovery on the 24th but does not confirm it. These are ordinary factual details rather than legal dispositions, so item 3's court-record standard does not govern them: what closes them is reliable supporting evidence someone has read directly — a charging document, a police or medical examiner's record, or a contemporaneous account from a source with direct knowledge. A court record is one route, not a requirement. The body text names neither, so no sentence in the book is exposed, but the endnote carries both as reporting and the manuscript's silence does not establish them. An earlier draft of this report treated this item as settled by that silence. It is not, and it is restored here.
